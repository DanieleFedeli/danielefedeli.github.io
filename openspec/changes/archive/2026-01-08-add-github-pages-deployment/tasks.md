# Implementation Tasks

## Phase 1: Setup GitHub Actions Workflow
- [x] Create `.github/workflows/` directory
- [x] Create `deploy.yml` workflow file with Zola build and GitHub Pages deployment steps
- [x] Configure workflow to trigger on push to main branch
- [x] Set up permissions for GitHub Pages deployment

## Phase 2: Configure Base URL
- [x] Determine GitHub Pages URL format (danielefedeli.github.io)
- [x] Update `base_url` in `config.toml` to match GitHub Pages URL
- [x] Verify all internal links use `get_url()` helper (already done)

## Phase 3: Repository Setup
- [x] Connect local repository to GitHub remote
- [x] Add `.gitignore` for build artifacts (public/)
- [x] Push all changes to GitHub
- [ ] Document GitHub Pages setup instructions (Settings → Pages → Source: gh-pages branch)
- [ ] Add deployment status badge to README (optional)

## Phase 4: Testing
- [ ] Verify GitHub Actions workflow runs successfully
- [ ] Verify site is accessible at GitHub Pages URL
- [ ] Test navigation, search, and all features on deployed site

## Phase 5: Documentation
- [ ] Update project.md with deployment information
- [ ] Document how to trigger manual deployments if needed
